...
#+BEGIN_SRC emacs-lisp
  ...
  (when t
          ...
          (global-set-key (kbd "M-# M-s") 
                                                                          (lambda()(interactive) (find-file "~/emacs/org-notes/LinuxAdoption.org")))
...
#+BEGIN_SRC emacs-lisp
  (eval-after-load "grep"
    '(grep-apply-setting
      'grep-find-command
      "find . ! -name \"*~\" ! -name \"#*#\" ! -path \"*/.svn/*\" -type f -print0 | xargs -0 -e grep -nH -e " )
    )
#+END_SRC
...
+ gu, gU :: change case of selection
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq org-agenda-files '("~/cbi/config/"))
  ...
  (setq org-directory "~/emacs/org-notes")
  (setq org-mobile-inbox-for-pull "~/emacs/org-notes/mobile_inbox.org")
  (setq org-mobile-directory "/scpc:root@2.josg.de:~/MobileOrg")
...
#+BEGIN_SRC emacs-lisp
  (setq org-capture-templates                                          
        '(("t" "Todo" entry (file+headline "~/emacs/org-notes/LinuxAdoption.org" "Tasks")
           "* TODO %?\n  %i\n  %a")
          ("j" "Journal" entry (file+datetree "~/emacs/org-notes/journal.org")
           "* %?\nEntered on %U\n  %i\n  %a")
          ))
  #+END_SRC
** alte Notizeqn
...
*** DONE MobileOrg
:PROPERTIES:
:ID:       ffaddf39-fbb0-4fc2-9485-a441ae236f48
:END:
**** Shortcut:
C-c C-x RET p (push) g (pull)
**** TODO BUG: "Capture as node/child"'s changes in mobileorg.org are not integrated
:PROPERTIES:
:ID:       754f8434-c20b-42c8-ba01-6ac6b90248da
:END:
...
** Script
#+BEGIN_SRC emacs-lisp
;; AUCTeX was installed using ELPA package.el
...
#+BEGIN_SRC emacs-lisp
  (when t
    ...
    ;; needs Mozrepl Plugin: https://addons.mozilla.org/de/firefox/addon/mozrepl/
...
** Autosave: http://www.gnu.org/software/emacs/manual/html_node/emacs/Recover.html#Recover
** Backup
...
#+BEGIN_SRC emacs-lisp
  ...
  (when t
    ...
    (defun dotemacs ()
      (interactive)
      (find-file-other-window "~/cbi/config/emacs.org")
      )
...
#+BEGIN_SRC emacs-lisp
  ...
  ;;Speck (for in buffer spelling checks)
  (when t
    (require 'speck)
    )
