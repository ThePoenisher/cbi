...
(defun jo/copy-file-name-to-clipboard ()
...
*** Slow Point Movement
**** examples:
***** very very long 27000 lines (like sinlge line html files in grep search) in truncate-lines mode
moving down and up, even in fundamental mode
without global-hl-line-mode it is only slow going up
***** visual selecting in org mode in evil at the end of a 17000 line long file
**** Hooks?
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; not working when issues this early (fset 'color-theme-jo (color-theme-make-snapshot)) ;; to revert to old theme
...
#+BEGIN_SRC emacs-lisp
  ...
  (add-to-list 'auto-mode-alist '("\\.mkd$"  markdown-mode t))
#+END_SRC
*** Garbage collection
...
****** Block Visual (rectangular region)
******* Enter with C-v and then: I (insert) A (append),
******* use $ for lines of differnet lengths
******* move past ends: virtualedit 
http://stackoverflow.com/questions/10772598/vim-select-the-ends-of-multiple-lines-block-mode-but-where-the-ending-column-v
******* evil (not available)
use picture or artist mode
http://emacs.stackexchange.com/questions/7272/simulate-vims-virtualedit-and-nostartofline
http://stackoverflow.com/questions/7019131/how-can-i-draw-to-an-xy-position-in-emacs/18995901
***** Eigene
#+BEGIN_SRC emacs-lisp
  ...
  (evil-define-key 'motion help-mode-map "l" 'evil-undefine)
  (evil-define-key 'motion info-mode-map "l" 'evil-undefine)
...
***** performance: sehr langsam in org files mit umlauten in überschrift
verbringt sehr viel zeit in. line-number-at-pos
**** and Org
...
C-h C <RET>
*** Finding the Unicode codepoint of a character
describe-char
...
insert-char
http://superuser.com/questions/394405/how-to-type-a-unicode-character-by-its-number-in-emacs
** Local Variables
...
**** (setq word-wrap t) (most users set it through visual line mode)
**** longlines-mode 
...
http://www.emacswiki.org/emacs/init-multiple-cursors.el
**** in Evil
https://github.com/magnars/multiple-cursors.el/issues/17
http://www.reddit.com/r/emacs/comments/2gx5s3/any_good_tips_to_get_multiplecursors_with_evil/
**** alternatives?
http://www.emacswiki.org/emacs/multiple-line-edit.el
**** Problem [2015-03-04 Wed]
makes region resize with arrow keys very slow
*** Mark regions
**** rectangular
** Bugs und Warnings
...
** Commands:
*** read only: C-x C-q
** auto-revert-mode (auch global)
** open several files: C-x C-f glob
** Mode Line
*** Problem: Mail is shown because $MAIL is not empty due to mailbox format
#+BEGIN_SRC emacs-lisp
...
[[file:/usr/share/emacs/24.5/lisp/org/org-agenda.el.gz::(defun%20org-search-view]]     
after: (while (setq file (pop files))
**** http://www.chemie.fu-berlin.de/chemnet/use/info/elisp/elisp_31.html
*** no-properties (without formatting...)
M-x apropos RET no-properties RET
*** org-map-*
*** good sources
http://orgmode.org/worg/org-hacks.html
**** Check for misplaced SCHEDULED and DEADLINE cookies in subtree
** Performance
...
#+END_SRC
** directories / tree / recursive
ediff-directories

anderes zeug
http://www.emacswiki.org/emacs/EdiffTrees
http://www.emacs.uniyar.ac.ru/doc/em24h/emacs124.htm
* DONE LaTeX
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (eval-after-load 'bibtex
    '(progn    
       ...
       ;; (setq bibtex-autokey-name-change-strings 
       ;;       '(("\\\\aa" . "a")
       ;;         ("\\\\AA" . "A")
       ;;         ("\\\"a\\|\\\\\\\"a\\|\\\\ae" . "ae")
       ;;         ("\\\"A\\|\\\\\\\"A\\|\\\\AE" . "Ae")
...
***** immernoch [2014-12-10 Wed]
#+BEGIN_SRC fundamental
  (/usr/share/texmf-dist/tex/latex/tools/xr.sty) (./abkuerzungen.tex
  *************************************************
  * LaTeX warning: "xparse/redefine-command"
  * 
  * Redefining document command \ast with arg. spec. ' O{t} ' on line 117.
  *************************************************
  ) (./paper2.aux) (/usr/share/texmf-dist/tex/context/base/supp-pdf.mkii
#+END_SRC
** Script
#+BEGIN_SRC emacs-lisp
  ;; muss man anders setzen und was soll das überhaupt
  ...
  (eval-after-load 'latex
    '(progn    
        ...
        (define-key LaTeX-mode-map
          [f4] (tex-build-command-function '(jo/TeX-Bib-command) nil t))
        (define-key LaTeX-mode-map
  ...
  (defun jo/TeX-Bib-command ()
    (interactive)
    (if LaTeX-biblatex-use-Biber
        TeX-command-Biber
      TeX-command-BibTeX))

  ;; warum habe ich eine eigene? weil die vorhandene C-c C-c immer nachfragt
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (org-add-link-type "b" 'ebib-open-org-link)
  (eval-after-load 'ebib
    '(progn    
       (setq ebib-preload-bib-files
             '(
               "/home/data/promotion/TEX/manuell.bib"
               "/home/data/promotion/Literatur/literatur.bib"
               "/home/data/promotion/TEX/MendeleyBibtexSync/library.bib"
               ))
       (setq ebib-use-timestamp t)
       (setq ebib-index-display-fields '(;;"author"
                                         "year" "title" "journal"))
       (setq ebib-autogenerate-keys t)
       (setq ebib-index-window-size 20)
       (setq ebib-file-search-dirs '("/home/data/promotion/Literatur/"))
       ...
       ;;(add-hook 'ebib-entry-mode-hook  'toggle-truncate-lines)
       (add-hook 'ebib-entry-mode-hook 'visual-line-mode)

       ;; modification from uptream: this version uses first command, without promptin
  ...
  completion works."
            ...
            (let ((collection (ebib--create-collection-from-db)))
              (when collection
                (let* ((key (completing-read "Key to insert: " collection nil t nil 'ebib--key-history))
                       ...
                       (citation-command
                        (ebib--ifstring (format-string (cadr (car format-list)))
       ...
       (defun jo/ebib-show-from-org-subtree ()
         (interactive)
         (let ((keys (jo/org-extract-references-from-subtree)))
           (ebib-db-set-filter
            `(contains "=key=" ,(mapconcat 'identity keys "\\|"))
            ebib--cur-db)
           (ebib--redisplay)
           ))
  ...
  (defun jo/org-extract-references-from-subtree ()
    (let ((res '()))
      (org-map-tree
       (lambda ()
         (re-search-forward "^\\** \\(.*\\)$")
         ;; (message (concat "headline: " (match-string 1)))
         (goto-char (match-beginning 1))
         (let* ((limit (match-end 1)))
           ;; (print limit)
           (while (re-search-forward "\\<b:\\(\\w+\\)\\>" limit t)
             (push (match-string-no-properties 1) res)
             ))))
      res
      ))
#+END_SRC
** auto key generations: see bibtex-generate-autokey
** insert into buffer
*** database not loaded
[[info:ebib#Associating%20a%20Database%20with%20a%20LaTeX%20File][info:ebib#Associating a Database with a LaTeX File]]
*** ebib-local-bibtex-filenames
** import from url
*** https://gist.github.com/mardukbp/6576427
...
#+END_SRC
*** https://gist.github.com/mardukbp/6576418
#+BEGIN_SRC emacs-lisp
(require 'mm-url)
...
#+END_SRC
*** https://github.com/joostkremers/ebib/issues/23
**** https://gist.github.com/mardukbp/6567775
#+BEGIN_SRC emacs-lisp
...
(defun ebib-import-arxiv (arxiv-url)
  ...
  (let ((tempbuff (get-buffer-create "*arxiv*"))
    ...
    (with-current-buffer tempbuff
      ...
      (kill-buffer (current-buffer)))))
#+END_SRC
**** https://gist.github.com/mardukbp/6567792
#+BEGIN_SRC javascript
...
#+END_SRC
** Filter recently added entries  https://github.com/joostkremers/ebib/issues/24
* Git (in emacs)
...
- SEHR WICHTIG
  ...
  -   (ido-vertical-mode) ;changes a lot of key bindings!!!!!!!
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; (ido-vertical-mode 1) ;changes a lot of key bindings!!!!!!!
  ...
  (setq ido-max-work-file-list 10000)
  (setq ido-max-dir-file-cache 10000)

  (ido-sort-mtime-mode 1)  ;; (old comment: maybe better use flx?)
...
#+END_SRC
**** sort by mtime
http://emacswiki.org/emacs/InteractivelyDoThings#toc25
or better: use ido-sort-mtime package
#+BEGIN_SRC emacs-lisp :tangle no
...
C-c C-b 
C-b C-z
*** TODO Definitions etc.
see Settings

more: https://github.com/haskell/haskell-mode/wiki/Haskell-Interactive-Mode-Tags
*** Block Based indentation and region commands
   ...
   (eval-after-load 'haskell-mode
     '(progn    
        ...
        (setq haskell-process-suggest-overloaded-strings nil)
         ;;     "sh -c \"cd `upfind -name .cabal-sandbox`; cabalrepl\"")
        )
     )
...
#+END_SRC
*** Formatting
https://github.com/haskell/haskell-mode/wiki/Autoformatting
https://github.com/jaspervdj/stylish-haskell
*** Tags
https://github.com/haskell/haskell-mode/wiki/Haskell-Interactive-Mode-Tags
*** GEIL: shm, structured-haskell-mode
https://github.com/chrisdone/structured-haskell-mode
**** Problem: [2014-09-28 Sun]
buggy (vielleicht wegen evil?) und zu speziell?
*** TODO: Debug
http://youtu.be/WAtEmoVN30E?list=UUeoDK51Iln7Wk7QwDdoa9VA
...
- M-: :: eval-buffer
- C-M-arrows or p/n/f/b/k :: navigation, but problematic (due to
     point) in evil
...
#+END_SRC
** C++
*** switch/jump between header and object file
M-x ff-find-other-file
** CSS Colors
http://ergoemacs.org/emacs/emacs_CSS_colors.html
http://www.emacswiki.org/emacs/CssColor
*** Rainbow-mode
...
emacs-lisp-mode or c-mode. The hexadecimal ones (#aabbcc) are always
activated.
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; the following 'is likely to cause problems' (emacs init warning) 
